...
# number of samples (imagninary participants)
n <- 20
...
Plot inindividual subject's choice
```{r}
...
Function to plot one simulation 
```{r}
plot_choice <- function(simulation,caps) {
  n = length(caps)
  for (sub_idx in 1:n) {
...
```{r}
plot_choice(simulation.caps, caps)
```
